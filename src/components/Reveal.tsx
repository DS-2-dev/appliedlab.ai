...
// page ends on (Ask the Lab, above the footer).
...
export function Reveal({
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      const out = leave ? clamp((bottom - LEAVE_TO * vh) / ((LEAVE_FROM - LEAVE_TO) * vh)) : 1;
      const v = Math.min(enter, out);
      const shift = (out - enter) * DRIFT;
