  ...
  ({ id, label, icon: Icon, badge, disabled, className, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "group/tree relative flex h-8 cursor-pointer items-center justify-between gap-2 rounded-md px-2.5 pl-8 text-left text-sm outline-none transition-colors select-none",
          ...
          {Icon && (
            <Icon
              className={cn(
                ...
                isSelected
                  ? "text-foreground"
                  : "text-muted-foreground group-hover/tree:text-foreground"
              )}
            />
          )}

          <span className="truncate leading-none transition-colors duration-150 group-hover/tree:text-foreground">
            {label}
          </span>
        </div>
  ...
  return (
    <div className={cn("flex flex-col gap-0.5", className)} {...props}>
      <button
        ...
        onKeyDown={handleKeyDown}
        className={cn(
          "group/tree relative flex h-8 cursor-pointer items-center justify-between gap-2 rounded-md px-2.5 pl-8 text-left text-sm font-normal text-muted-foreground outline-none select-none",
          "focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1",
          disabled && "cursor-not-allowed pointer-events-none opacity-40"
        )}
      >
        ...
        <div className="relative z-10 flex min-w-0 items-center gap-2">
          <Icon className="size-4 shrink-0 text-muted-foreground transition-colors duration-150 group-hover/tree:text-foreground" />
        ...
        <div className="relative z-10 flex items-center gap-1">
          ...
          <ChevronRight
            className={cn(
              "size-3.5 shrink-0 text-muted-foreground transition-transform duration-200 group-hover/tree:text-foreground",
  ...
  return (
    <div className={cn("flex flex-col gap-0.5", className)}>
      <button
        ...
        onClick={handleToggle}
        onKeyDown={handleKeyDown}
        // Projectum: the heading reads like the sidebar's own entries (All
        // Projects): same face, size, case, weight, colour and height. Hover
        // only darkens the text. The tree's groups are named (group/tree):
        // shadcn's sidebar root is an unnamed group, so a bare group-hover
        // would light every row whenever anything in the bar is hovered.
        className="group/tree flex h-8 w-full cursor-pointer items-center justify-between rounded-md px-2 text-left text-sm text-sidebar-foreground/70 outline-none transition-colors hover:text-sidebar-accent-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring"
      >
        <span className="truncate">{title}</span>

        <ChevronDown
          className={cn(
            "size-3.5 text-muted-foreground transition-transform duration-200 group-hover/tree:text-foreground",
            !isExpanded && "-rotate-90"
          )}
        />
      </button>
...
// onSelect is the tree's own (an item id), so the DOM event handler of the
